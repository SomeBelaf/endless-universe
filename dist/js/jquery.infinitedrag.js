...
(function ($) {
  ...
  var InfiniteDrag = function InfiniteDrag(draggable, draggable_options, tile_options) {
    ...
    var _do = draggable_options ? draggable_options : {}; // Tile options (DEFAULT)
    ...
    function create_tile(i, j) {
      ...
      var x = i * _to.width;
      var y = j * _to.height;
      var $e = $draggable.append('<pre></pre>');
    ...
    var update_containment = function update_containment() {
      // Update viewport info.
      viewport_width = $viewport.width(), viewport_height = $viewport.height(), viewport_cols = Math.ceil(viewport_width / _to.width), viewport_rows = Math.ceil(viewport_height / _to.height); // Create containment box.
    ...
    var update_tiles = function update_tiles() {
      var $this = $draggable;
      var $parent = $this.parent(); // Problem with .position() in Chrome/WebKit:
      ...
      for (var _i = visible_left_col; _i <= visible_left_col + viewport_cols; _i++) {
        for (var j = visible_top_row; j <= visible_top_row + viewport_rows; j++) {
          if (grid[_i] === undefined) {
            grid[_i] = {};
          } else if (grid[_i][j] === undefined) {
            create_tile(_i, j);
          }
        }
      }
    ...
    self.move = function (col, row) {
      ...
      if (containment[0] <= new_offset.left && new_offset.left <= containment[2] && containment[1] <= new_offset.top && new_offset.top <= containment[3]) {
    ...
    for (var _i2 = _to.start_col, m = _to.start_col + viewport_cols; _i2 < m && _to.range_col[0] <= _i2 && _i2 <= _to.range_col[1]; _i2++) {
      grid[_i2] = {};
