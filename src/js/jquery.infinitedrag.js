...
(function($) {
	 ...
	 */
	let InfiniteDrag = function(draggable, draggable_options, tile_options) {
		// Use self to reduce confusion about this.
		let self = this;

		let $draggable = $(draggable);
		let $viewport = $draggable.parent();
		$draggable.css({
			position: "relative",
		});
		
		// Draggable options
		let _do = (draggable_options) ? draggable_options : {};
		...
		function create_tile(i, j) {
			...
			let x = i * _to.width;
			let y = j * _to.height;
			let $e = $draggable.append('<pre></pre>');

			let $new_tile = $e.children(":last");
			$new_tile.attr({
		...
		let update_containment = function() {
		...
		let update_tiles = function() {
			...
			let visible_left_col = Math.ceil(-pos.left / _to.width) - 1,
				visible_top_row = Math.ceil(-pos.top / _to.height) - 1;

			for (let i = visible_left_col; i <= visible_left_col + viewport_cols; i++) {
				for (let j = visible_top_row; j <= visible_top_row + viewport_rows; j++) {
					if (grid[i] === undefined) {
						grid[i] = {};
		...
		self.move = function(col, row) {
			...
			let containment = $draggable.draggable("option", "containment");
		...
		let grid = {};
		for (let i = _to.start_col, m = _to.start_col + viewport_cols; i < m && (_to.range_col[0] <= i && i <= _to.range_col[1]); i++) {
			grid[i] = {}
			for (let j = _to.start_row, n = _to.start_row + viewport_rows; j < n && (_to.range_row[0] <= j && j <= _to.range_row[1]); j++) {
				create_tile(i, j);
			}
		}
